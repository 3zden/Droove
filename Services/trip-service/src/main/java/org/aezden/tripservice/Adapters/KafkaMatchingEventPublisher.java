...
@Slf4j
public class KafkaMatchingEventPublisher implements TripEventPublisher<MatchRequest> {
    private final KafkaTemplate<String, MatchRequest> kafkaTemplate;


    @Override
    public void publish(MatchRequest event) {
        log.info("Matching event published Successfully!, and the trip now is: Matching");
        Message<MatchRequest> message = MessageBuilder
                .withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, "match-requests")
                .build();
